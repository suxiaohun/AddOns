...
local CN_WAN=10000
local CN_SHIWAN=100000
local CN_YI=100000000
local CN_ZHAO=1000000000000
local CN_JING=10000000000000000

kui.num = function(num)
    if not num then return end
    if num < CN_WAN then
        return num
    elseif num >= CN_JING then
        return string.format('%.2f京', num/CN_JING)
    elseif num >= CN_ZHAO then
        return string.format('%.2f兆', num/CN_ZHAO)
    elseif num >= CN_YI then
        return string.format('%.2f亿', num/CN_YI)
    elseif num >= CN_SHIWAN then
        return string.format('%.2f万', num/CN_WAN)
    elseif num >= CN_WAN then
        return string.format('%.1f万', num/CN_WAN)
    end
end
