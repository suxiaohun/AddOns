...
--先从列表中移除当前角色已经存在的坐骑
for i = 1, test.GetNumCompanions("MOUNT") do
    local id, name, _, _, _, typeID, typeString = test.GetCompanionInfo("mount", i)
...
for k, v in pairs(boss_table) do
    ...
    if location and location == v[1] then
        ...
        list = list..scolor.Sprintf("underline")..(scolor.Sprintf("red",location))..scolor.Sprintf("clear").."\n"
    end
    if raid and raid == v[2] then
        ...
        list = list..scolor.Sprintf("bold")..(scolor.Sprintf("green","    " .. raid .. "(" .. v[3] .. ")")).."\n"
    end
    print.pink("        [" .. v[4] .. "]\027[93m-->["..v[5].."%]")
    list = list..scolor.Sprintf("pink","        [" .. v[4] .. "]\027[93m-->["..v[5].."%]").."\n"
